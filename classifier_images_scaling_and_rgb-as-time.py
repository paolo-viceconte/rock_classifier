...
if __name__ == "__main__":
            ...
            if dataset in train_datasets:
                ...
                for j in range(len(data[str(hand)+"_palm"])):
                    ...
                    if labels[j] != -1:
                        ...
                        train_image = np.zeros((9, 11, 3))
                        for i in range(len(image_ordered_palm_x)):
                            rgb_as_sequence = [data[str(hand)+"_palm"][j - 2 * k_downsampling][i]/255,
                                               data[str(hand)+"_palm"][j - k_downsampling][i]/255,
                                               data[str(hand)+"_palm"][j][i]/255]
                            train_image[image_ordered_palm_x[i], image_ordered_palm_y[i]] = rgb_as_sequence
            ...
            if dataset in test_datasets:
                ...
                for j in range(len(data[str(hand) + "_palm"])):
                    ...
                    if labels[j] != -1:
                        ...
                        test_image = np.zeros((9, 11, 3))
                        for i in range(len(image_ordered_palm_x)):
                            rgb_as_sequence = [data[str(hand)+"_palm"][j - 2 * k_downsampling][i]/255,
                                               data[str(hand)+"_palm"][j - k_downsampling][i]/255,
                                               data[str(hand)+"_palm"][j][i]/255]
                            test_image[image_ordered_palm_x[i], image_ordered_palm_y[i]] = rgb_as_sequence
    ...
    if training:
        ...
        model = keras.Sequential(
            [
                keras.layers.InputLayer(input_shape=(9, 11, 3)),
    ...
    for i in range(len(x_test)):
        pred = model(np.reshape(x_test[i], (1, x_test[i].shape[0], x_test[i].shape[1], x_test[i].shape[2]))).numpy()[0]
